...
// Función para agregar un producto al carrito con botón de eliminar
function addToCart(event) {
    var button = event.target;
    var product = button.closest('.card');
    ...
    var productName = product.querySelector('.PRENDA').innerText;
    var productPrice = parseFloat(product.querySelector('.precio').innerText.replace('$', ''));
    var productSize = product.querySelector('select').value;

    // Crear contenedor para el producto con botón de eliminar
    var cartItem = document.createElement('div');
    cartItem.className = 'cart-item';

    var cartText = document.createElement('span');
    cartText.innerText = `${productName} - Talla: ${productSize} - $${productPrice.toFixed(2)}`;
    ...
    deleteButton.addEventListener('click', function () {
        // Eliminar el producto del contenedor y de la lista
        cartItem.remove();
        selectedProducts = selectedProducts.filter(item => item !== productObj);
        updateCartTotal();
    });
    ...
    // Agregar el producto a la lista de productos seleccionados
    var productObj = { name: productName, price: productPrice, size: productSize };
